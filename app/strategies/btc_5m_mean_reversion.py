...
class BtcMeanReversionStrategy(Strategy):
    ...
    def _quote_size(self, inventory: Decimal, side: OrderSide) -> Decimal:
        base = min(self.config.quote_size, self.config.max_quote_size)
        ...
        if sell_size > ZERO and Decimal("0") < ask_price < Decimal("1"):
            quotes.append(
                QuoteIntent(
                    ...
                    neg_risk=market.neg_risk,
                    post_only=True,
                    reason=f"mean_reversion_ask; prob={prob:.4f}",
                )
            )
        return quotes
